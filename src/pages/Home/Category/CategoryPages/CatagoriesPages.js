...
const CatagoriesPages = () => {
  const { name } = useParams();
  const [colum, SetColumn] = useState("4");
  const Proudcts = [...AllProducts];

  const filtteredProducts = Proudcts.filter(
    (products) => products.productCatgoris === name
  );
  ...
  //This is Main Products Array That shows products catagoris wise
  const [products, SetProducts] = useState(filtteredProducts);
  ...
  // Function used for store checkbox value.
  const handleClick = (e) => {
  ...
  useEffect(() => {
    //Delivary Filter Start
    if (freeDelivary === true && ExpressDelivary === true) {
      const filteredProductsbasedOnDelivery = filtteredProducts.filter(
        (product) =>
      ...
      SetProducts(filtteredProducts);
    } else {
      return filtteredProducts;
    }
    //Delivary Filter End
  }, [freeDelivary, ExpressDelivary, filtteredProducts]);

  useEffect(() => {
    //Size Wise Filter Start
    const filterProductsAccordingtoSize = filtteredProducts.filter(
    ...
    //Size Wise Filter End
  }, [Size, filtteredProducts]);

  useEffect(() => {
    //Price Wise Filter Start

    //only products Price
    const ProductsPrice = filtteredProducts.map((product) => {
    ...
    const filteredProductsbasedOnPrice = filtteredProducts.filter(
      (product) => parseInt(product.productPrice) <= filteredPrice
    );
    SetfiltredProducts(filteredProductsbasedOnPrice);
  }, [filtteredProducts, filteredPrice]);
  ...
  if (FilterValueHighAndLow === "high") {
    ...
    filtteredProducts.sort(compairfuntion);
  }
          ...
          <div className="col-span-2 mt-3">
            <div className="bg-white rounded-lg mb-4 mx-3">
              ...
              <ul className="mx-2 pb-2">
                <Link to={"/categories"}>
                  <li className="text-sm pb-1">All</li>
                </Link>
                <Link to={"/categories/mansfashion"}>
                  <li className="text-sm pb-1">Man Fashion</li>
                </Link>
                <Link to={"/categories/womensfashion"}>
                  <li className="text-sm pb-1">Woman Fashion</li>
                </Link>
                <Link to={"/categories/kidsfashion"}>
                  <li className="text-sm pb-1">Kids Fashion</li>
                </Link>
                <Link to={"/categories/voucher"}>
                  <li className="text-sm pb-1">Voucher</li>{" "}
                </Link>
                <Link to={"/categories/sports"}>
                  <li className="text-sm pb-1">Sports</li>
                </Link>
                <Link to={"/categories/gadget"}>
                  <li className="text-sm pb-1">Gadget</li>{" "}
                </Link>
                <Link to={"/categories/coumputer&laptop"}>
                  <li className="text-sm pb-1">Computer & Laptop</li>{" "}
                </Link>
                <Link to={"/categories/game"}>
                  <li className="text-sm pb-1">Games</li>{" "}
                </Link>
                <Link to={"/categories/property"}>
                  <li className="text-sm pb-1">Property</li>{" "}
                </Link>
              </ul>
            </div>
            <div className="bg-white rounded-lg mb-4 mx-3">
...
export default CatagoriesPages;
